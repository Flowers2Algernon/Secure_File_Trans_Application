...
class EncrptedFileAdmin(admin.ModelAdmin):
    list_display = ('file_id', 'original_filename', 'file_size', 'uploaded_date', 'code_expire')
...
class FileLogAdmin(admin.ModelAdmin):
    list_display = ('encrptedFile', 'download_time', 'download_final_datetime', 'ip_address')
...
class FileRequestAdmin(admin.ModelAdmin):
    list_display = ('request_id', 'requester_email', 'requester_name', 'created_at', 'expires_at', 'is_fulfilled')
